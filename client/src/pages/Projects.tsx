...
export default function ProjectsPage() {
  ...
  const queue = trpc.buildforge.builds.create.useMutation({
    onSuccess: (build) => {
      toast.success(`Build inserido na fila · posição ${build.queuePosition}`);
      void utils.buildforge.builds.list.invalidate();
      void utils.buildforge.dashboard.summary.invalidate();
    },
    onError: (error) => toast.error(error.message),
  });
  const removeProject = trpc.buildforge.projects.delete.useMutation({
    onSuccess: () => {
      void utils.buildforge.projects.list.invalidate();
      void utils.buildforge.builds.list.invalidate();
      void utils.buildforge.dashboard.summary.invalidate();
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // As mutações já apresentam a mensagem de erro adequada.
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-slate-950/55 p-0 backdrop-blur-sm sm:items-center sm:justify-center sm:p-6">
          <form onSubmit={submit} className="w-full max-w-2xl rounded-t-3xl bg-white p-6 shadow-2xl dark:bg-slate-950 sm:rounded-3xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-semibold text-slate-950 dark:text-white">Importar projeto</h2>
                <p className="mt-1 text-sm text-slate-500">A plataforma identifica a stack preliminar e prepara a fila. Repositórios Git são clonados e inspecionados pelo worker seguro antes do build.</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Fechar importação">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="mt-6 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1 dark:bg-slate-900">
              {(Object.keys(sourceCopy) as Source[]).map((item) => (
                <button type="button" key={item} onClick={() => setSource(item)} className={`rounded-xl px-3 py-3 text-sm font-semibold transition ${source === item ? "bg-white text-indigo-600 shadow-sm dark:bg-slate-800 dark:text-indigo-300" : "text-slate-500"}`}>
                  {sourceCopy[item].title}
                </button>
              ))}
            </div>
            <p className="mt-2 text-xs text-slate-500">{sourceCopy[source].hint}</p>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Nome do projeto<input value={name} onChange={(event) => setName(event.target.value)} required minLength={2} placeholder="Meu aplicativo" className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none ring-indigo-500 transition focus:ring-2 dark:border-slate-700 dark:bg-slate-900" /></label>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Branch<input value={branch} onChange={(event) => setBranch(event.target.value)} placeholder="main" className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none ring-indigo-500 transition focus:ring-2 dark:border-slate-700 dark:bg-slate-900" /></label>
            </div>
            {source !== "zip" && <label className="mt-4 block text-sm font-medium text-slate-700 dark:text-slate-300">URL do repositório<input value={reference} onChange={(event) => setReference(event.target.value)} required type="url" placeholder={source === "github" ? "https://github.com/organizacao/app" : "https://git.exemplo.com/equipe/app.git"} className="mt-1.5 h-11 w-full rounded-xl border border-slate-300 bg-white px-3 text-sm outline-none ring-indigo-500 transition focus:ring-2 dark:border-slate-700 dark:bg-slate-900" /></label>}
      ...
      {projects.isLoading ? <div className="grid min-h-64 place-items-center"><LoaderCircle className="h-6 w-6 animate-spin text-indigo-500" /></div> : projects.data?.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {projects.data.map((project) => (
            <article key={project.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
              <div className="flex items-start justify-between gap-3"><span className="rounded-lg bg-indigo-500/10 p-2 text-indigo-600 dark:text-indigo-300"><FolderGit2 className="h-5 w-5" /></span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold capitalize text-slate-600 dark:bg-slate-800 dark:text-slate-300">{frameworkLabel(project.framework)}</span></div>
              <h2 className="mt-5 truncate text-lg font-semibold text-slate-950 dark:text-white">{project.name}</h2>
              <p className="mt-1 min-h-10 text-sm text-slate-500 dark:text-slate-400">{project.description || project.repoUrl || "Projeto sem descrição"}</p>
              <div className="mt-4 flex items-center gap-2 text-xs text-slate-500"><GitBranch className="h-3.5 w-3.5" />{project.branch} · {project.buildCount} build(s)</div>
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-slate-800">
                <span className="text-xs text-slate-500">Atualizado {formatDate(project.updatedAt)}</span>
