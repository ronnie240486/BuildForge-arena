...
function storeWebsiteStarterFiles(title: string) {
  const safeTitle = title.replace(/[<>]/g, "").slice(0, 120) || "Loja Profissional";
  const base = commercialStarterFiles("website", safeTitle).filter((file) => !["src/main.ts", "src/style.css", "STUDIO_PRODUCT_STANDARD.md"].includes(file.filePath));
  const appName = JSON.stringify(safeTitle);
  return [...base,
    { filePath: "src/data/catalog.ts", language: "typescript", content: `export type Product={id:string;name:string;price:number;description:string;accent:string};export const catalog:Product[]=[{id:"bolsa",name:"Bolsa Nômade",price:189,description:"Design autoral e acabamento premium.",accent:"#c47846"},{id:"vela",name:"Vela Âmbar",price:59,description:"Aroma marcante para o seu espaço.",accent:"#c49a56"},{id:"cadeira",name:"Cadeira Mínima",price:449,description:"Conforto, madeira e linhas essenciais.",accent:"#8a6953"}];export const money=(value:number)=>new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(value);` },
    { filePath: "src/services/cart.ts", language: "typescript", content: `import type{Product}from"../data/catalog";const key="store-cart-v1";export const loadCart=():Product[]=>{try{return JSON.parse(localStorage.getItem(key)??"[]")}catch{return[]}};export const saveCart=(items:Product[])=>localStorage.setItem(key,JSON.stringify(items));` },
    { filePath: "src/main.ts", language: "typescript", content: `import"./style.css";import{catalog,money,Product}from"./data/catalog";import{loadCart,saveCart}from"./services/cart";const name=${appName};const root=document.getElementById("app");let cart:Product[]=loadCart();function render(){if(!root)return;const total=cart.reduce((sum,item)=>sum+item.price,0);root.innerHTML=\`<main class="store"><header><div><p>COMÉRCIO DIGITAL · CATÁLOGO</p><h1>\${name}</h1></div><button id="cart">Sacola \${cart.length}</button></header><section class="hero"><span>COLEÇÃO ESSENCIAL</span><h2>Peças que acompanham seu ritmo.</h2><p>Escolha produtos, organize o carrinho e siga para uma experiência de compra clara.</p></section><section class="catalog">\${catalog.map(product=>\`<article class="product"><div class="visual" style="--accent:\${product.accent}"><span>✦</span></div><h3>\${product.name}</h3><p>\${product.description}</p><div><strong>\${money(product.price)}</strong><button data-add="\${product.id}">Adicionar</button></div></article>\`).join("")}</section><section id="drawer" class="drawer" aria-live="polite"></section></main>\`;document.querySelectorAll<HTMLButtonElement>("[data-add]").forEach(button=>button.onclick=()=>{const product=catalog.find(item=>item.id===button.dataset.add);if(product){cart=[...cart,product];saveCart(cart);render()}});document.getElementById("cart")?.addEventListener("click",()=>{const drawer=document.getElementById("drawer");if(!drawer)return;drawer.innerHTML=cart.length?\`<h2>Seu carrinho</h2>\${cart.map(item=>\`<div class="cart-line"><span>\${item.name}</span><strong>\${money(item.price)}</strong></div>\`).join("")}<div class="cart-line total"><span>Total</span><strong>\${money(total)}</strong></div><button id="checkout">Ir para checkout</button>\`:\`<h2>Sua sacola está vazia</h2><p>Adicione produtos do catálogo para continuar.</p>\`;document.getElementById("checkout")?.addEventListener("click",()=>{const checkout=document.getElementById("checkout");if(checkout)checkout.textContent="Checkout pronto para integrar pagamento ✓"})})}render();` },
    { filePath: "src/style.css", language: "css", content: `:root{font-family:Inter,system-ui,sans-serif;color:#1c1714;background:#f6f4ef}*{box-sizing:border-box}body{margin:0}.store{width:min(1120px,100%);margin:auto;padding:28px 20px 70px}header{display:flex;justify-content:space-between;gap:20px;align-items:center}header p,.hero span{margin:0;color:#9b5d2e;font-size:11px;font-weight:900;letter-spacing:.14em}h1{margin:6px 0;font-size:clamp(28px,5vw,48px);letter-spacing:-.06em}button{border:0;border-radius:12px;padding:11px 14px;background:#29201b;color:#fff;font-weight:800;cursor:pointer}.hero{margin-top:23px;padding:clamp(25px,6vw,65px);border-radius:28px;background:linear-gradient(125deg,#e8b478,#a9562b);color:#fff}.hero span{color:#fff0e8}.hero h2{max-width:560px;margin:10px 0;font-size:clamp(34px,7vw,66px);line-height:.95;letter-spacing:-.065em}.hero p{max-width:470px;margin:0;color:#fff3e9;line-height:1.6}.catalog{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:24px}.product{padding:14px;border:1px solid #e4dad2;border-radius:22px;background:#fff}.visual{height:185px;display:grid;place-items:center;border-radius:15px;background:radial-gradient(circle,var(--accent) 0 30%,transparent 31%),#f4e5d8;color:#fff;font-size:55px}.product h3{margin:14px 0 5px}.product p{min-height:40px;margin:0;color:#846c5d;font-size:13px;line-height:1.5}.product>div:last-child{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:14px}.product>div:last-child button{background:#e76f35;font-size:11px}.drawer{margin-top:20px}.drawer:not(:empty){padding:20px;border:1px solid #e4dad2;border-radius:22px;background:#fff}.drawer h2{margin:0 0 8px}.cart-line{display:flex;justify-content:space-between;gap:10px;padding:11px 0;border-bottom:1px solid #efe6de}.cart-line.total{font-size:18px;border:0}.drawer button{margin-top:12px;width:100%}@media(max-width:680px){.catalog{grid-template-columns:1fr 1fr}.store{padding:20px 12px 55px}}@media(max-width:390px){.catalog{grid-template-columns:1fr}header{align-items:flex-start;flex-direction:column}}` },
    { filePath: "docs/WEB_CHECKOUT.md", language: "markdown", content: "# Checkout web\n\nO carrinho usa armazenamento local para a experiência inicial. Antes de produção, conecte um provedor de pagamentos no servidor, valide preço e estoque no backend e confirme pedidos por webhook." },
    { filePath: "STUDIO_PRODUCT_STANDARD.md", language: "markdown", content: "# Padrão de Loja Web\n\nPreservar catálogo, carrinho, valor total, ação de checkout e integração segura de pagamento antes da venda em produção." },
  ];
}
...
export function studioStarterFiles(projectType: "website" | "application", name: string) {
  ...
  if (projectType === "website") return isStoreStudioProject(title) ? storeWebsiteStarterFiles(title) : commercialStarterFiles(projectType, title);
...
export function studioPreviewPreferenceFile(message: string, files: Array<{ filePath: string; content: string }>) {
  ...
  const isRacing = /hot\s*wheels|corrida|racing|pista|velocidade|turbo|chamas?|automotivo/.test(source);
  ...
  if (isCalculator && (isScientific || isRacing || requestedColor || requestedBrand || hasUniversalRequest)) {
    return { filePath: "studio-preview.json", language: "json", content: JSON.stringify({ ...withUniversal, calculator: { scientific: isScientific || current.calculator?.scientific, racing: isRacing || current.calculator?.racing } }, null, 2) };
  }
  if (isAgenda && (requestedColor || requestedBrand)) {
    const primary = requestedColor ?? current.agenda?.primary ?? "violet";
    return { filePath: "studio-preview.json", language: "json", content: JSON.stringify({ ...withUniversal, agenda: { primary, accent: primary === "yellow" ? "pink" : "yellow", brandName: requestedBrand?.slice(0, 60) ?? current.agenda?.brandName } }, null, 2) };
  }
  ...
  if (!isGame && (requestedColor || requestedBrand || hasUniversalRequest)) {
...
export async function applyStudioChatEdit(input: { actor: PlatformActor; projectId: number; message: string; preferredModel?: string }) {
  ...
  if (previewPreference) {
    ...
    if (changed.length === 0) throw new Error("Esta configuração já está aplicada na prévia. Peça uma mudança diferente para continuar.");
    ...
    for (const file of changed) await db.insert(studioFiles).values({ studioProjectId: project.id, ...file }).onDuplicateKeyUpdate({ set: { language: file.language, content: file.content } });
    const reply = "A configuração foi persistida em studio-preview.json e a prévia será recarregada com a alteração visível solicitada.";
    await db.insert(studioMessages).values({ studioProjectId: project.id, authorId: input.actor.id, role: "assistant", content: reply, changedFiles: changed.map((file) => file.filePath) });
    const sync = await syncStudioFilesToGithub(db, project, changed);
    await addAuditLog({ actorId: input.actor.id, action: "studio.chat_visual_edit", entityType: "studio_project", entityId: String(project.id), metadata: { files: changed.map((file) => file.filePath), githubSync: sync.status, githubPushed: sync.pushed } });
    return { reply, changedFiles: changed.map((file) => file.filePath), model: "configuração visual determinística", sync };
  }
