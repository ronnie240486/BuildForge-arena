...
export function createStudioPreviewDocument(preview: StudioPreview) {
  ...
  return (index?.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace("</head>", `<style>${styles}</style></head>`).replace(/<div\s+id=["']app["']\s*><\/div>/i, `<div id="app">${extractedBody}</div>`)) ?? `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>${styles}</style></head><body>${extractedBody}</body></html>`;
}
